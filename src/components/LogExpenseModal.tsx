    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24 }}>
          <div>
            <h2 style={{ fontSize: 20, fontWeight: 600, color: 'var(--text-primary)' }}>Log Expense</h2>
            <p style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 4 }}>
              Available: <span style={{ color: 'var(--text-secondary)', fontWeight: 600 }}>{formatNu(currentBalance)}</span>
            </p>
          </div>
          <button
            onClick={onClose}
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            {parsedAmount > 0 && (
              <div style={{
                ...
                <p style={{ fontSize: 13, color: afterBalance < 0 ? 'var(--red)' : 'var(--green)', fontWeight: 500 }}>
              ...
              {QUICK_AMOUNTS.slice(0, 6).map(a => (
                <button
                  ...
                  style={{
                    ...
                    fontSize: 14,
                    fontWeight: 500,
                    background: amount === String(a) ? 'var(--red-dim)' : 'var(--bg-muted)',
                    borderColor: amount === String(a) ? 'var(--red)' : 'var(--border)',
                    color: amount === String(a) ? 'var(--red)' : 'var(--text-secondary)',
                  }}
                >
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
              {EXPENSE_CATEGORIES.map(cat => (
                ...
                >
                  ...
                  <span style={{ fontSize: 11, fontWeight: categoryId === cat.id ? 600 : 400, textAlign: 'center' }}>
                    {cat.name}
                  </span>
                </button>
              ))}
